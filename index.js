...
for (var i = 0; i < totalMonths; i++) {
  var month = finances[i][0]; // each month
  var amount = finances[i][1]; // each amount of the month

  // calculate total net/
  netTotal += amount; // total amount of the months

  // changes in Profit/Losses calculation

  var changes = amount - previousMonthAmount;
  totalChange += changes;
  previousMonthAmount = amount;

  // tracking greatest increase and decrease over the entire period.
  if (changes > greatestIncrease.amount) {
    greatestIncrease.amount = changes;
    greatestIncrease.date = month;
  };

  if (changes < greatestDecrease.amount) {
    greatestDecrease.amount = changes;
    greatestDecrease.date = month;
  }
}

// the average changes in Profit/Losses
var totalAverage = (totalChange / (totalMonths - 1)).toFixed(2)      // round up to 2dp
